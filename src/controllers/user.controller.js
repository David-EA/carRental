...
const signup = async (req, res)=> {
    ...
    try{
        ...
        //create jwt
        const token = await jwt.sign({email}, process.env.JWT_SECRET, {expiresIn: process.env.JWT_EXPIRATION || '1d'});
        ...
        const newUser = new User({
            ...
            token: token // Store the token in the user document(not advisable according to tutor)
        });
        await newUser.save();
...
const login = async (req, res)=> {
    ...
    try{
        ...
        const token = await jwt.sign({id: user._id}, process.env.JWT_SECRET, {expiresIn: process.env.JWT_EXPIRATION || '1d'});
...
const makeAdmin = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'User promoted to admin successfully', data: user });
    } catch (error) {
